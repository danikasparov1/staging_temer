...
class CrmLeadCallCenter(models.Model):
    ...
    full_phone_ids = fields.Many2many(
        ...
        compute="_compute_full_phone_ids",
        store=False,
    ...
    @api.depends('country_id', 'phone_number')
    def _compute_full_phone(self):
        for record in self:
            if record.country_id and record.phone_number:
                country_code = record.country_id.phone_code or ''
                record.full_phone = f'+{country_code}{record.phone_number}'
            else:
                record.full_phone = ''
                
    @api.depends('full_phone')
    def _compute_full_phone_ids(self):
        """Compute the IDs of all registered phone numbers to exclude them from the selection."""
        all_phone_ids = self.env['crm.callcenter.phone'].search([]).ids
        for record in self:
            record.full_phone_ids = all_phone_ids
